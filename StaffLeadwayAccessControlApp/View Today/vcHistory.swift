...
class vcHistory: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate{
    ...
    override func viewDidLoad() {
        ...
        //tblAllAppoitnments.rowHeight = 90
    ...
    func GetAppointments(){
        SVProgressHUD.show()
        let todoEndpoint: String = "http://dynamics.somee.com/api/Staff/ViewStaffAppointmentsForToday/\(staffID)"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        {
            let cell = UITableViewCell(style: .default, reuseIdentifier: "cell")
            cell.textLabel?.text = "No Appointments Today"
            return cell
        }
        else
        {
            ...
            Alamofire.request(todoEndpoint)
                .responseJSON { response in
                    ...
                    if(status){
                        ...
                        let push = vcSpecificAppointment(nibName:"vcSpecificAppointment", bundle:nil)
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        searchresult = pp.filter({$0.AccessCode!.lowercased().prefix(searchText.count) == searchText.lowercased() || $0.CompanyName!.lowercased().prefix(searchText.count) == searchText.lowercased() || $0.visFName!.lowercased().prefix(searchText.count) == searchText.lowercased() || $0.visSName!.lowercased().prefix(searchText.count) ==
            searchText.lowercased() || $0.Date!.lowercased().prefix(searchText.count) ==
            searchText.lowercased() || $0.Time!.lowercased().prefix(searchText.count) == searchText.lowercased() || $0.Status!.lowercased().prefix(searchText.count) == searchText.lowercased()})
